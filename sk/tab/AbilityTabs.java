...
public enum AbilityTabs implements Tab {
	...
	private AbilityTabs(final int c, final int... setVals) {
		this.component = c;
	...
	public boolean isOpen() {
		return MainTabs.ABILITY_BOOK.isOpen() && Arrays.binarySearch(setVals, Settings.get(SETTING_ID, 0,
				SETTING_MASK)) >= 0;
	}
	...
	public boolean open() {
		WidgetChild wc;
		return MainTabs.ABILITY_BOOK.open() && (this.isOpen() || (wc = Widgets.get(BOOK_WIDGET,
				component)) != null && wc.visible() && wc.click(true) && new TimedCondition(2000) {
			@Override
	...
	public static AbilityTabs getCurrent() {
		for (final AbilityTabs t : values()) {
			if (t.isOpen()) {
				return t;
			}
		}
